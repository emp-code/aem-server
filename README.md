...
All-Ears uses a custom five-bit encoding (Addr32) to fit all addresses into 10 bytes. The Addr32 form is used to create a 8-byte hash, which is what the server stores. On receiving mail, the address is hashed, and the hash is checked to see if a user owns it.
