...
The `private` data field can be used by clients to store 3613 bytes of arbitrary data. Its contents are sent with each `account/browse` API request, and it can be updated using the `private/update` API. The server does nothing with the data, and it can technically be used by clients for any purpose. The official clients encrypt it with libsodium's Sealed Box using the user's public key, and use it to store Address/Gatekeeper/Contacts data.
...
All-Ears uses a custom five-bit encoding (Addr32) to fit all addresses into 10 bytes. The Addr32 form is used to create a 8-byte Argon2 hash, which is stored by All-Ears.
