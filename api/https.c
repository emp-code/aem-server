...
int tlsSetup(mbedtls_x509_crt * const tlsCert, mbedtls_pk_context * const tlsKey) {
	...
	if (ret != 0) {syslog(LOG_MAIL | LOG_ERR, "mbedtls_ssl_config_defaults failed: %m"); return -1;}
	...
	if (ret != 0) {syslog(LOG_MAIL | LOG_ERR, "mbedtls_ctr_drbg_seed failed: %m"); return -1;}

	ret = mbedtls_ssl_conf_own_cert(&conf, tlsCert, tlsKey);
	if (ret != 0) {syslog(LOG_MAIL | LOG_ERR, "mbedtls_ssl_conf_own_cert failed: %m"); return -1;}

	ret = mbedtls_ssl_setup(&ssl, &conf);
	if (ret != 0) {syslog(LOG_MAIL | LOG_ERR, "mbedtls_ssl_setup failed: %m"); return -1;}
...
void respond_https(int sock) {
	...
	while ((ret = mbedtls_ssl_handshake(&ssl)) != 0) {
		if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
			syslog(LOG_MAIL | LOG_INFO, "mbedtls_ssl_handshake failed: %d", ret);
