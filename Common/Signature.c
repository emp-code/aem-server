...
void aem_sign_message(unsigned char * const msg, const size_t lenMsg, const unsigned char usk[AEM_USK_KEYLEN]) {
	...
	unsigned char uHash[53];
	crypto_generichash((unsigned char*)uHash, 53, msg + 26, lenMsg - 26, usk, AEM_USK_KEYLEN); // Skip 26*8=208 of the 212 signature bits (remaining four bits are zero)
	unsigned char sHash[27];
	crypto_generichash((unsigned char*)sHash, 27, uHash,    53,          ssk, AEM_SSK_KEYLEN);
...
bool aem_sig_verify(const unsigned char uHash[53], const unsigned char sHash_test[27]) {
	...
	crypto_generichash((unsigned char*)sHash_real, 27, uHash, 53, ssk, AEM_SSK_KEYLEN);
	sHash_real[26] &= 15;
