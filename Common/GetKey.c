...
#include "../Common/AEM_KDF.h" // For AEM_KDF_SMK_KEYLEN
...
int getKey(unsigned char * const master) {
	...
	char masterHex[AEM_KDF_SMK_KEYLEN * 2];
	...
	sodium_hex2bin(master, AEM_KDF_SMK_KEYLEN, masterHex, AEM_KDF_SMK_KEYLEN * 2, NULL, NULL, NULL);
	sodium_memzero(masterHex, AEM_KDF_SMK_KEYLEN * 2);
	return 0;
}
