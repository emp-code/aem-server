...
static int64_t addressToHash(const char sixBit[16], const unsigned char key[16]) {
	unsigned char hash16[16];
	crypto_generichash(hash16, 16, (unsigned char*)sixBit, 16, key, 16);
...
int addressToPublicKey(const char sixBit[16], unsigned char pk[32], const unsigned char key[16]) {
	sqlite3* db;
	if (sqlite3_open_v2("Data/Users.aed", &db, SQLITE_OPEN_READONLY, NULL) != SQLITE_OK) return -1;

	sqlite3_stmt* query;
//	int ret = sqlite3_prepare_v2(db, "SELECT ownerpk FROM address WHERE hash=?", -1, &query, NULL);
	int ret = sqlite3_prepare_v2(db, "SELECT ownerpk FROM address LIMIT 1", -1, &query, NULL);
//	sqlite3_bind_int64(query, 1, addressToHash(sixBit, key));
...
unsigned char *getUserMessages(const unsigned char pk[32], size_t *totalSize) {
	sqlite3* db;
	if (sqlite3_open_v2("Data/Messages.aed", &db, SQLITE_OPEN_READONLY, NULL) != SQLITE_OK) return NULL;

	sqlite3_stmt* query;
	int ret = sqlite3_prepare_v2(db, "SELECT msg FROM messages WHERE ownerpk=? ORDER BY rowid DESC LIMIT 1", -1, &query, NULL);
...
int addUserMessage(const unsigned char ownerPk[32], const unsigned char *msgData, const size_t msgLen) {
	sqlite3* db;
	if (sqlite3_open_v2("Data/Messages.aed", &db, SQLITE_OPEN_READWRITE, NULL) != SQLITE_OK) return -1;
